...
override.controller('editOverride',function($scope,$httpWrapper,$routeParams,$queryFilter,$breadcrumb,$menu,$dialog){
    ...
    $httpWrapper.post({
        url:"override/provider/"+encodeURIComponent(encodeURIComponent($scope.serviceKey))+"/methods",
        success:function(data){
            $scope.methods=data;
    ...
    $scope.save=function(){
        ...
        $httpWrapper.post({
            url:"override/provider/"+encodeURIComponent(encodeURIComponent($scope.serviceKey))+"/saveOverride",
            data:item,
            success:function(data){
...
override.controller('providerOverrides',function($scope,$httpWrapper,$routeParams,$queryFilter,$breadcrumb,$menu,$dialog){
    ...
    var queryConfig=function(){
        $httpWrapper.post({
            url:"override/provider/"+encodeURI($routeParams.serviceKey)+"/list",
            success:function(data){
                $scope.details=data;
                if(!data||data.length<=0){
                    $scope.isEmpty=true;
    ...
    var queryWeightConfig = function(){
        $httpWrapper.post({
            url:"override/provider/"+encodeURIComponent($routeParams.serviceKey)+"/weight-list",
            success:function(data){
                $scope.details=data;
                if(!data||data.length<=0){
                    $scope.isEmpty=true;
    ...
    var queryLoadBalance=function(){
        $httpWrapper.post({
            url:"override/provider/"+encodeURIComponent($routeParams.serviceKey)+"/loadbalance-list",
            success:function(data){
                $scope.details=data;
                if(!data||data.length<=0){
                    $scope.isEmpty=true;
